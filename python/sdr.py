...
try:
    ...
    # configure device
    sdr.sample_rate = 2.56e6  # Max sample rate of rtl-sdr is 3.2 Mhz
    ...
    matplotlib.pyplot.show()                        # show psd graph

    while True:
        matplotlib.pyplot.clf()
        samples = sdr.read_samples(256*1024) # read samples from sdr
        matplotlib.pyplot.psd(samples, NFFT=1024, Fs=sdr.sample_rate/1e6, Fc=sdr.center_freq/1e6) # create power spectral density graph
